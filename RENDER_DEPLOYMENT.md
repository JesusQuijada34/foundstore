...
La rama `render` aísla este servicio Flask de la aplicación React/tRPC que permanece en `main`. En esta rama, `app.py`, `maintenance.py`, `requirements.txt`, `test_app.py`, `render.yaml` y `render_templates/` viven en la raíz. Render debe desplegar desde la raíz con `python app.py`; el módulo inicia Waitress, un servidor WSGI, en el puerto que Render entrega mediante `$PORT`.
